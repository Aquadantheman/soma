...
def _aggregate_nightly_sleep(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Rename for clarity
    nightly = nightly.rename(columns={
        'sleep_rem': 'rem_min',
        'sleep_deep': 'deep_min',
        'sleep_core': 'core_min',
    ...
    nightly['total_sleep_min'] = nightly['rem_min'] + nightly['deep_min'] + nightly['core_min']
...
def compute_sleep_deviation(
    night: NightlySleep,
    baseline: SleepArchitectureBaseline
...
def analyze_sleep_trend(
    ...
    if not is_significant:
        direction = 'stable'
        interpretation = f"{metric} has remained stable over the past {period_days} days"
    elif slope > 0:
        direction = 'improving' if metric in ['rem_pct', 'deep_pct', 'efficiency'] else 'increasing'
        interpretation = f"{metric} is {direction} ({slope:.2f} per day, p={p_value:.3f})"
    else:
        direction = 'declining' if metric in ['rem_pct', 'deep_pct', 'efficiency'] else 'decreasing'
        interpretation = f"{metric} is {direction} ({slope:.2f} per day, p={p_value:.3f})"
...
def generate_sleep_report(
    ...
    if len(nightly) > 0:
        ...
        for _, row in recent_data.iterrows():
            recent_nights.append(NightlySleep(
                date=row['sleep_date'],
                rem_min=float(row['rem_min']),
                deep_min=float(row['deep_min']),
                core_min=float(row['core_min']),
    ...
    if baseline:
        ...
        if baseline.rem_pct.mean < 15:
            concerns.append(f"Average REM% ({baseline.rem_pct.mean:.1f}%) is below typical range (20-25%)")
        elif baseline.rem_pct.mean >= 20:
            insights.append(f"Healthy REM% ({baseline.rem_pct.mean:.1f}%)")

        if baseline.deep_pct.mean < 10:
            concerns.append(f"Average deep sleep ({baseline.deep_pct.mean:.1f}%) is below typical range (13-23%)")
        ...
        if baseline.efficiency.mean < 85:
            concerns.append(f"Sleep efficiency ({baseline.efficiency.mean:.1f}%) is below 85% threshold")
        elif baseline.efficiency.mean >= 90:
            insights.append(f"Excellent sleep efficiency ({baseline.efficiency.mean:.1f}%)")

        if not baseline.is_stable:
            concerns.append("Sleep patterns are inconsistent (high night-to-night variability)")
        else:
            insights.append(f"Consistent sleep patterns (stability score: {baseline.consistency_score:.0f}/100)")
