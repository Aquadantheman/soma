...
def analyze_spo2(
    ...
    night_pct_below_95 = (
        float(np.sum(night_values < 95) / len(night_values) * 100)
    ...
    has_concerning = (
        pct_below_95 > 20
        or pct_below_90 > 5
        or night_pct_below_95 > 30
        or (diff > 1.5 and abs(cohens_d) > 0.5)  # Significant night dip
    )
...
def analyze_mobility(
    ...
    ws_df = pd.DataFrame({"time": pd.to_datetime(ws_times), "value": ws_values})
    ...
    if asymmetry_series is not None and len(asymmetry_series.dropna()) >= 10:
        ...
        asym_alert = (
            float(np.mean(recent_asym)) > 10 if len(recent_asym) >= 5 else False
        )
    else:
        asym_mean = None
        asym_alert = False
    ...
    interpretation_parts.append(f"Walking speed ({ws_mean:.2f} m/s) is {ws_status}.")
    ...
    if st_status != "unknown":
        risk_level = {"ok": "low", "low": "moderate", "very_low": "elevated"}.get(
            st_status, "unknown"
        )
        interpretation_parts.append(
...
def compute_data_adequacy(
    signals: pd.DataFrame, min_samples: dict[str, int] = None
) -> list[DataAdequacy]:
...
def compute_correlation(
    x: pd.Series, y: pd.Series, min_samples: int = 30
...
def compute_detrended_correlation(
    x: pd.Series, y: pd.Series, window: int = 30, min_samples: int = 30
) -> tuple[float, float, int]:
...
def compute_lagged_correlation(
    x: pd.Series, y: pd.Series, lag_days: int, min_samples: int = 30
) -> tuple[float, float, int]:
...
def detect_compensatory_exercise(
    weight: pd.Series, activity: pd.Series, min_samples: int = 30
...
def detect_weekend_warrior(
    activity: pd.Series, min_weeks: int = 4
) -> Optional[BehavioralPattern]:
...
def detect_seasonal_activity_pattern(
    activity: pd.Series, min_months: int = 6
) -> Optional[BehavioralPattern]:
...
def detect_all_behavioral_patterns(
    signals_by_biomarker: dict[str, pd.Series], min_samples: int = 30
...
def find_cross_domain_interconnections(
    ...
    for source_bio, target_bio, source_domain, target_domain in relationships_to_test:
        if (
            source_bio not in signals_by_biomarker
            or target_bio not in signals_by_biomarker
        ):
            continue
        ...
        for lag in range(0, max_lag + 1):
            r, p, n = compute_lagged_correlation(
                source_data, target_data, lag, min_samples
            )
...
def compute_cardiovascular_score(
    ...
    if "spo2" in signals_by_biomarker:
        ...
        if len(spo2_data) >= 14:
            ...
            _overall_spo2 = spo2_data.mean()  # noqa: F841
            ...
            if recent_spo2 >= 97 and pct_below_95 < 5:
                ...
                limiting_factors.append(
                    f"Frequent low SpO2 readings ({pct_below_95:.0f}% below 95%)"
                )
            elif recent_spo2 < 95:
                limiting_factors.append(
                    f"Blood oxygen below optimal ({recent_spo2:.1f}%)"
                )
...
def compute_sleep_score(
    ...
    if duration_data is not None and len(duration_data) >= 7:
        ...
        if 420 <= avg_duration <= 540:
            ...
            duration_score = 60
            limiting_factors.append(
                f"Excessive sleep duration ({avg_duration/60:.1f} hrs)"
            )
        else:
            duration_score = 75
    ...
    if "sleep_efficiency" in signals_by_biomarker:
        ...
        if len(efficiency_data) >= 7:
            ...
            if avg_efficiency >= 90:
                efficiency_score = 95
                contributors.append(
                    f"Excellent sleep efficiency ({avg_efficiency:.0f}%)"
                )
                ...
                efficiency_score = 70
                limiting_factors.append(
                    f"Moderate sleep efficiency ({avg_efficiency:.0f}%)"
                )
            else:
                efficiency_score = 50
                limiting_factors.append(
                    f"Poor sleep efficiency ({avg_efficiency:.0f}%)"
                )
    ...
    if (
        "sleep_deep" in signals_by_biomarker
        and duration_data is not None
        and len(duration_data) >= 7
    ):
        ...
        if len(common_idx) >= 7:
            deep_pct = (
                deep_data.loc[common_idx] / duration_data.loc[common_idx] * 100
            ).mean()
    ...
    if "steps" in signals_by_biomarker:
        ...
        if len(steps_data) >= 7:
            ...
            if avg_steps >= 12000:
                steps_score = 95
                contributors.append(
                    f"Excellent activity level ({avg_steps:.0f} steps/day)"
                )
            ...
            if len(steps_data) >= 14:
                cv = (
                    steps_data.std() / steps_data.mean()
                    if steps_data.mean() > 0
                    else 1.0
                )
                if cv < 0.3:
    ...
    if "exercise_time" in signals_by_biomarker:
        ...
        if len(exercise_data) >= 7:
            ...
            if avg_exercise >= 30:
                exercise_score = 95
                contributors.append(
                    f"Excellent exercise time ({avg_exercise:.0f} min/day)"
                )
                ...
                exercise_score = 45
                limiting_factors.append(
                    f"Limited structured exercise ({avg_exercise:.0f} min/day)"
                )
...
def compute_body_composition_score(
    ...
    trend = "stable"
    if (
        body_composition_report is not None
        and body_composition_report.trend is not None
    ):
    ...
    if mobility_analysis is not None:
        ...
        if mobility_analysis.steadiness_mean is not None:
            ...
            if st_status == "ok":
                st_score = 90
                contributors.append(
                    f"Low fall risk (steadiness {mobility_analysis.steadiness_mean:.1f}%)"
                )
            elif st_status == "low":
                st_score = 55
                limiting_factors.append(
                    f"Moderate fall risk (steadiness {mobility_analysis.steadiness_mean:.1f}%)"
                )
            else:  # very_low
                st_score = 25
                limiting_factors.append(
                    f"Elevated fall risk (steadiness {mobility_analysis.steadiness_mean:.1f}%)"
                )
...
def compute_wellness_score(
    ...
    body_composition = compute_body_composition_score(
        signals_by_biomarker, body_composition_report
    )
    ...
    harmonic_mean = (
        total_weight / weighted_reciprocal_sum if weighted_reciprocal_sum > 0 else 50.0
    )
    ...
    # =========================================================================
    arithmetic_mean = (
        sum(w * s for w, s in zip(weights, scores)) / total_weight
        if total_weight > 0
        else 50.0
    )
    ...
    if max_score > bottleneck_score:
        ...
        simulated_harmonic = (
            total_weight / simulated_reciprocal_sum
            if simulated_reciprocal_sum > 0
            else 50.0
        )
...
def synthesize_risk_factors(
    ...
    if wellness_score.mobility is not None:
        ...
        if wellness_score.mobility.trend == "declining":
            ...
            declining_in_factors = any(
                "declining" in f.lower() for f in mobility_limiting
            )
            if not declining_in_factors:
                risk_factors.append(
                    RiskFactor(
                        ...
                        level="moderate",
                        contributing_factors=[
                            "Walking speed declining over time (confound-controlled)"
                        ],
                        trend="worsening",
                        modifiable=True,
    ...
    for ic in interconnections:
        if ic.relationship == "negative" and ic.strength == "strong":
            ...
            if (
                ic.source_domain == "body_composition"
                and ic.target_domain == "cardiovascular"
            ):
                if "body_mass" in ic.pathway.lower() and "vo2" in ic.pathway.lower():
                    # Weight negatively affecting fitness
                    if not any(
                        rf.name == "Body Composition Concern" for rf in risk_factors
                    ):
                        risk_factors.append(
                            RiskFactor(
                                ...
                                level="moderate",
                                contributing_factors=[
                                    "Higher weight associated with lower VO2 Max"
                                ],
                                trend="stable",
                                modifiable=True,
...
def generate_recommendations(
        ...
        if rf.level in ("elevated", "high"):
            if "cardiovascular" in rf.name.lower():
                if any("vo2" in f.lower() for f in rf.contributing_factors):
                    recommendations.append(
                        domain_recommendations["cardiovascular"]["low_vo2"]
                    )
                if any("hrv" in f.lower() for f in rf.contributing_factors):
                    recommendations.append(
                        domain_recommendations["cardiovascular"]["low_hrv"]
                    )
            ...
            if "body composition" in rf.name.lower():
                if any("gain" in f.lower() for f in rf.contributing_factors):
                    recommendations.append(
                        domain_recommendations["body_composition"]["weight_gain"]
                    )
    ...
    for limiting in wellness_score.cardiovascular.limiting_factors:
        ...
        if "rhr" in limiting.lower():
            if (
                domain_recommendations["cardiovascular"]["high_rhr"]
                not in recommendations
            ):
                recommendations.append(
                    domain_recommendations["cardiovascular"]["high_rhr"]
    ...
    for limiting in wellness_score.sleep.limiting_factors:
        ...
        if "efficiency" in limiting.lower():
            if domain_recommendations["sleep"]["low_efficiency"] not in recommendations:
                recommendations.append(
                    domain_recommendations["sleep"]["low_efficiency"]
                )
    ...
    for limiting in wellness_score.activity.limiting_factors:
        ...
        if "inconsistent" in limiting.lower():
            if (
                domain_recommendations["activity"]["inconsistent"]
                not in recommendations
            ):
                recommendations.append(
                    domain_recommendations["activity"]["inconsistent"]
