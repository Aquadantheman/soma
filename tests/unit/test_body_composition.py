...
def make_weight_data(n_measurements: int = 30, base_weight: float = 75.0,
...
def make_body_composition_data(n_measurements: int = 30, base_weight: float = 75.0,
                                base_bf: float = 20.0, seed: int = 42) -> pd.DataFrame:
    ...
    def test_female_body_fat_higher_than_male(self):
        ...
        for age_range, percentiles in ACSM_BODY_FAT_PERCENTILES.items():
            for pct, (male, female) in percentiles.items():
                assert female > male, f"Female should be higher at {age_range}, {pct}th percentile"
