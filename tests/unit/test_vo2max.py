...
def make_vo2max_data(n_measurements: int = 30, base_value: float = 40.0,
...
def make_correlated_data(n_days: int = 60, vo2_base: float = 40.0,
...
class TestComputeFitnessAge:
    ...
    def test_interpretation_positive_difference(self):
        ...
        result = compute_fitness_age(50.0, 50, 'male')
        assert 'younger' in result.interpretation.lower() or 'excellent' in result.interpretation.lower()
    ...
    def test_interpretation_negative_difference(self):
        ...
        assert 'older' in result.interpretation.lower() or 'below' in result.interpretation.lower() or 'improvement' in result.interpretation.lower()

    def test_includes_hunt_study_reference(self):
        """Should cite HUNT Fitness Study."""
        result = compute_fitness_age(40.0, 40, 'male')
        assert 'Nes' in result.reference or 'HUNT' in result.reference or '2013' in result.reference
...
class TestComputeMortalityRisk:
    ...
    def test_includes_kodama_reference(self):
        ...
        result = compute_mortality_risk(40.0)
        assert 'Kodama' in result.reference or 'JAMA' in result.reference or '2009' in result.reference
...
class TestAssessTrainingResponse:
    ...
    def test_returns_training_response_object(self):
        ...
        measurements = [
            VO2MaxMeasurement(date=date.today() - timedelta(days=i*7), value=40.0, mets=11.4)
            for i in range(10)
        ]
    ...
    def test_detects_high_responder(self):
        ...
        for i in range(5):
            measurements.append(VO2MaxMeasurement(
                date=base_date + timedelta(days=120 + i*30),  # Days 120, 150, 180, 210, 240
                value=42.0,
                mets=42.0/3.5
            ))
    ...
    def test_male_values_higher_than_female(self):
        ...
        for age_range, percentiles in ACSM_PERCENTILES.items():
            for pct, (male, female) in percentiles.items():
                assert male > female, f"Male should be higher at {age_range}, {pct}th percentile"
